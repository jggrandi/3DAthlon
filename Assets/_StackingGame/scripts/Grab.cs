...
public class Grab : MonoBehaviour
{
    ...
    //-------------------------------------------------
    public void ProjectStackable(Stackable obj, int listSize, float lenghtRay)
    {

        if (obj.VisualRepresentation.GetComponent<Grab>().drawnPoints == null) return;
        ...
        foreach (var point in pS)
        {
            ...
            hits = Physics.RaycastAll(pointF, down, lenghtRay);
    ...
    private void OnHandHoverBegin(Hand hand)
    {
        ...
        foreach (var list in allStacks) {
            ...
            foreach (var l in list)
                lengthRay += l.transform.localScale.z;

            ProjectStackable(list[list.Count - 1], list.Count, lengthRay);
        }
        ...
        {// || ((hand.controller != null) && hand.controller.GetPressDown(Valve.VR.EVRButtonId.k_EButton_Grip))) {
            if (hand.currentAttachedObject != gameObject)
            {
                ...
                foreach (var list in allStacks)
                {
                    ...
                    foreach (var l in list)
                        lengthRay += l.transform.localScale.z;
                    if (list[list.Count - 1].VisualRepresentation.GetComponent<Grab>().drawnPoints != null)
                        Destroy(list[list.Count - 1].VisualRepresentation.GetComponent<Grab>().drawnPoints);


                    list[list.Count - 1].VisualRepresentation.GetComponent<Grab>().drawnPoints = new GameObject();
                    list[list.Count - 1].VisualRepresentation.GetComponent<Grab>().drawnPoints.transform.parent = list[list.Count - 1].VisualRepresentation.gameObject.transform;
                    ProjectStackable(list[list.Count - 1], list.Count, lengthRay);
                }
